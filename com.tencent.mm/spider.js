...
var mockerClass = "gz.wxvideo.SearchUIMocker7012";
...
//topActivityStartActivity("com.tencent.mm.plugin.finder.search.FinderFeedSearchUI")

function search(keyword,maxpage) {
    Java.perform(function() {
    	var SearchUIMockerClz = Java.use(mockerClass);
        var result = SearchUIMockerClz.search(keyword, maxpage);
        log(fastTojson(result));
    });
}
...
function startHttpServer() {
    Java.perform(function() {
        var SearchUIMockerClz = Java.use(mockerClass);
        SearchUIMockerClz.startHttpServer();
    });
}
startHttpServer();
function enterProfileUI(username) {
    Java.perform(function() {
        var SearchUIMockerClz = Java.use(mockerClass);
        SearchUIMockerClz.enterProfileUI(username);
    });
}


function finderMixSearchUI(keyword) {
    Java.perform(function() {
        var SearchUIMockerClz = Java.use(mockerClass);
        var result = SearchUIMockerClz.finderMixSearchUI(keyword, 1);
        log(fastTojson(result));
    });
}

function findFinderContacts(keyword) {
    Java.perform(function() {
        var SearchUIMockerClz = Java.use(mockerClass);
        var result = SearchUIMockerClz.findFinderContacts(keyword);
        log(fastTojson(result));
    });
}

function userFinderObjects(username) {
    Java.perform(function() {
        var SearchUIMockerClz = Java.use(mockerClass);
        var result = SearchUIMockerClz.userFinderObjects(username);
        log(fastTojson(result));
    });
}

//enterTagActivity("搞笑")
function enterTagActivity(tag) {
    Java.perform(function() {
        var SearchUIMockerClz = Java.use(mockerClass);
        var result = SearchUIMockerClz.enterTagActivity(tag);
        log(fastTojson(result));
    });
}
