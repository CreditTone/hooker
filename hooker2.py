...
def createHookingEnverment(packageName):
    if not os.path.exists(packageName):
        ...
        warn(f"Creating working directory: {packageName}")
...
tishi = """Please enter the debugging command.
show activity : obtains the information of the activity stack
show service : obtains the servic stack information
show object : views the internal information of the object according to objectId
hook {className}:{method} : Generate frida hook script for example: hook okhttp3.Request$Builder:addHeader"""
print(tishi)
hooker_cmd = ""
while True:
    try:
